...
class SimilarityEvaluator(object):
    ...
    def __init__(self, data, settings, rep, metric='tsd', ramp_io_perc=0.2):
        """constructor"""
...
# =============================================================================
        ...
        for i in range(0, matrix_len):
            for j in range(0, matrix_len):
                comp_sec = self.create_comparison_elements(simulation_data,
                                                           log_data, i, j)
                length = np.max([len(comp_sec['seqs']['s1']),
                                 len(comp_sec['seqs']['s2'])])
    ...
    def calculate_cost(self, times, s1_idx, s2_idx):
        if self.one_timestamp:
            ...
            if t1 > 0:
                b1 = (p1[s1_idx]/t1)
                cost = ((b1*np.abs(p2[s2_idx]-p1[s1_idx])) +
                        ((1 - b1)*np.abs(w2[s2_idx]-w1[s1_idx])))
            else:
                cost = 0
        return cost
...
# =============================================================================
    def dl_mae_distance(self, log_data, simulation_data):
        ...
        for i in range(0, matrix_len):
            for j in range(0, matrix_len):
                dl, mae = self.calculate_distances(simulation_data, log_data, i, j)
                dl_matrix[i][j] = dl
                mae_matrix[i][j] = mae
        # end = timer()
        # print(end - start)
    ...
    def calculate_distances(self, serie1, serie2, id1, id2):
        ...
        dl = jf.damerau_levenshtein_distance(''.join(serie1[id1]['profile']),
